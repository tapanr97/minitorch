...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        """
        variables_with_derivatives = []
        derivatives = wrap_tuple(cls.backward(ctx, d_output))
        for i in range(0, len(inputs)):
            if not is_constant(inputs[i]):
                variables_with_derivatives.append((inputs[i], inputs[i].expand(derivatives[i])))
        return variables_with_derivatives
...
def backpropagate(variable, deriv):
    ...
    while queue:
        ...
        variable_obj = variable_with_deriv[0]
        derivative = variable_with_deriv[1]

        if isinstance(variable_obj, Variable) and variable_obj.is_leaf():
            variable_obj.accumulate_derivative(derivative)
        else:
            history = variable_obj.history
            variable_with_derivs_after_backprop = history.backprop_step(derivative)
            for v in variable_with_derivs_after_backprop:
                if not is_constant(v[0]):
                    queue.append(v)
