...
def index_to_position(index, strides):
    ...
    """
    position = sum(zipWith(mul)(index, strides))
    return position
...
def to_index(ordinal, shape, out_index):
    ...
    """
    counter = 0
    for stride in strides_from_shape(shape):
        dimension = ordinal // stride
        ordinal = ordinal % stride
        out_index[counter] = dimension
        counter += 1
...
def broadcast_index(big_index, big_shape, shape, out_index):
    ...
    for i in range(len(shape)):
        out_index[i] = big_index[i + dim_diff] if shape[i] > 1 else 0
...
def shape_broadcast(shape1, shape2):
    ...
    if len1 < len2:
        for i in range(len2 - len1):
            temp_list.append(1)
        temp_list.extend(list(shape1))
        shape1_list = temp_list
    elif len2 < len1:
        for i in range(len1 - len2):
            temp_list.append(1)
        temp_list.extend(list(shape2))
        shape2_list = temp_list

    # Rule 2
    for i in range(len(shape2_list)):
        if shape1_list[i] != shape2_list[i]:
            if shape1_list[i] == 1:
                shape1_list[i] = shape2_list[i]
            elif shape2_list[i] == 1:
                shape2_list[i] = shape1_list[i]
            else:
                raise IndexingError("Error")
    return tuple(shape1_list)
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        permuted_strides = []
        for dim in order:
            permuted_shape.append(self.shape[dim])
            permuted_strides.append(self.strides[dim])

        permuted_shape = tuple(permuted_shape)
        permuted_strides = tuple(permuted_strides)

        return TensorData(self._storage, permuted_shape, strides=permuted_strides)
