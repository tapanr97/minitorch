...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    """
    add_vals = list(vals)
    add_vals[arg] += epsilon
    sub_vals = list(vals)
    sub_vals[arg] = sub_vals[arg] - epsilon
    fn_add_ans = f(*add_vals)
    fn_sub_ans = f(*sub_vals)
    diff = (fn_add_ans - fn_sub_ans) / (2 * epsilon)
    return diff
...
class Inv(ScalarFunction):
    ...
    def backward(ctx, d_output):
        a = ctx.saved_values
        return operators.mul(-d_output, operators.inv(math.pow(a, 2)))
